...
function createApp(config = process.env) {
  ...
  if (config.NODE_ENV === 'production') {
    if (config.DB_PATH.startsWith('/tmp/')) check(config.EPHEMERAL_STORAGE_CONFIRMED === 'yes', '무료 Render 임시 저장소 사용 확인 후 EPHEMERAL_STORAGE_CONFIRMED=yes를 설정하세요.');
    else check(config.PERSISTENT_STORAGE_CONFIRMED === 'yes', '영구 디스크 연결 확인 후 PERSISTENT_STORAGE_CONFIRMED=yes를 설정하세요.');
  }
  ...
  store.db.exec(`CREATE TABLE IF NOT EXISTS material_stock (
  ...
  const mergeSyncedMaterials=() => {
    ...
    for(const row of store.db.prepare('SELECT material_code,material_name,specification FROM material_stock ORDER BY material_code').all()) {
      ...
      catalog.materials.push({material_code:row.material_code,material_name:row.material_name,material_unit:1,specification:row.specification||'',visible:true});
  ...
  const requestInventorySync=() => store.tx(() => {
    ...
    const state=inventoryState();check(state.status!=='requested','이미 재고 동기화를 요청했습니다.');
    const requestId=crypto.randomUUID();store.setSetting('inventory_sync_status','requested');store.setSetting('inventory_sync_request_id',requestId);store.setSetting('inventory_sync_requested_at',Date.now());store.setSetting('inventory_sync_error','');return inventoryState();
  });
  ...
  const kioskCatalog=() => {
    ...
    return {...catalog,inventory_sync:inventoryState(),materials:catalog.materials.map(material=>{const stock=stockRows.get(material.material_code);if(!stock)return {...material,available_stock:null,stock_quantity:null,specification:material.specification||''};const reserved=(activeMap.get(material.material_code)||0)+(completedMap.get(material.material_code)||0);return {...material,specification:stock.specification||material.specification||'',stock_quantity:stock.stock_quantity,available_stock:Math.max(0,stock.stock_quantity-reserved),stock_synced_at:stock.synced_at};})};
  };
  ...
  app.use(helmet({contentSecurityPolicy:{directives:{defaultSrc:["'self'"],scriptSrc:["'self'","'unsafe-inline'",'https://ajax.googleapis.com','https://cdn.jsdelivr.net'],scriptSrcAttr:["'unsafe-inline'"],styleSrc:["'self'","'unsafe-inline'",'https://cdn.jsdelivr.net','https://fonts.googleapis.com'],fontSrc:["'self'",'https://fonts.gstatic.com'],imgSrc:["'self'",'data:','blob:'],connectSrc:["'self'"],frameAncestors:["'none'"]}},crossOriginEmbedderPolicy:false}));
  app.use(express.json({limit:'2mb'}));app.use('/public',express.static(path.join(__dirname,'public')));app.get('/ping',(req,res)=>res.send('pong'));app.get('/healthz',(req,res)=>{store.db.prepare('SELECT 1').get();res.json({ok:true});});
  ...
  app.get('/api/admin/backup',auth('ADMIN'),(req,res,next)=>{if(snapshotBusy)return res.status(409).json({error:'백업 생성 중입니다.'});snapshotBusy=true;const directory=fs.mkdtempSync(path.join(os.tmpdir(),'homself-backup-'));const file=path.join(directory,'homself.sqlite');const cleanup=()=>{fs.rmSync(file,{force:true});fs.rmdirSync(directory);snapshotBusy=false;};try{store.snapshot(file);res.download(file,'homself.sqlite',error=>{cleanup();if(error&&!res.headersSent)next(error);});}catch(error){cleanup();next(error);}});
...
if(require.main===module){const {app,store}=createApp();const server=app.listen(process.env.PORT||3000,()=>console.log('HOMSelf 시작: 승인 시트의 항목은 일괄 불출 시작 전까지 대기합니다.'));for(const signal of ['SIGINT','SIGTERM'])process.once(signal,()=>server.close(()=>{store.close();process.exit(0);}));}
